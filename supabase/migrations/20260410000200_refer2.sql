...
-- memori). Replace calmkids before applying.
-- ============================================================

set local search_path = calmkids, public;

-- 1. Referral codes (one per user, auto-generated)
create table if not exists calmkids.referral_codes (
...
create table if not exists calmkids.referrals (
...
create index if not exists referrals_referrer_idx
  on calmkids.referrals (referrer_user_id, status);

-- 3. Referral clicks (pre-signup visits — optional analytics)
create table if not exists calmkids.referral_clicks (
...
create table if not exists calmkids.referral_rewards (
  user_id uuid primary key references auth.users(id) on delete cascade,
...
-- ============================================================
create or replace function calmkids.generate_referral_code(p_user_id uuid)
returns text
language plpgsql
security definer
set search_path = calmkids, public, pg_temp
as $$
declare
  v_code text;
  v_attempts int := 0;
begin
  loop
    ...
    begin
      insert into calmkids.referral_codes (user_id, code)
  ...
  return (select code from calmkids.referral_codes where user_id = p_user_id);
end;
$$;
...
-- ============================================================
create or replace function calmkids.apply_referral(
...
begin
  ...
  if exists (select 1 from calmkids.referrals where referee_user_id = p_referee_user_id) then
    return jsonb_build_object('ok', false, 'reason', 'already_attributed');
  end if;

  insert into calmkids.referrals (referrer_user_id, referee_user_id, referee_email, code, status)
...
-- ============================================================
create or replace function calmkids.qualify_referral(
  p_referee_user_id uuid,
...
as $$
declare
  v_ref calmkids.referrals%rowtype;
  v_count int;
  v_reward calmkids.referral_rewards%rowtype;
  v_just_unlocked boolean := false;
begin
  select * into v_ref
  from calmkids.referrals
...
create or replace function calmkids.mark_referral_coupon_applied(
  p_user_id uuid,
  p_customer_id text,
...
-- (trigger on auth.users)
-- ============================================================
create or replace function calmkids.handle_new_user_referral_code()
returns trigger
language plpgsql
security definer
set search_path = calmkids, public, pg_temp
as $$
begin
  perform calmkids.generate_referral_code(new.id);
  return new;
end;
$$;

-- Each app needs its own trigger name so they don't collide
drop trigger if exists on_auth_user_created_calmkids_referral on auth.users;
create trigger on_auth_user_created_calmkids_referral
  after insert on auth.users
  for each row execute function calmkids.handle_new_user_referral_code();

-- ============================================================
-- RLS
-- ============================================================
alter table calmkids.referral_codes     enable row level security;
alter table calmkids.referrals          enable row level security;
alter table calmkids.referral_clicks    enable row level security;
alter table calmkids.referral_rewards   enable row level security;

-- Users can read their own code
drop policy if exists "own referral code" on calmkids.referral_codes;
create policy "own referral code" on calmkids.referral_codes
  for select using (auth.uid() = user_id);

-- Users can see referrals where they're the referrer
drop policy if exists "own referrals" on calmkids.referrals;
create policy "own referrals" on calmkids.referrals
  for select using (auth.uid() = referrer_user_id);

-- Users can see their own reward row
drop policy if exists "own reward" on calmkids.referral_rewards;
create policy "own reward" on calmkids.referral_rewards
  for select using (auth.uid() = user_id);

-- Clicks are write-only from the anon role (via API route), no select
drop policy if exists "no clicks read" on calmkids.referral_clicks;
create policy "no clicks read" on calmkids.referral_clicks
  for select using (false);

grant usage on schema calmkids to anon, authenticated, service_role;
grant select on calmkids.referral_codes, calmkids.referrals, calmkids.referral_rewards to authenticated;
grant insert on calmkids.referral_clicks to anon, authenticated;
grant execute on function calmkids.apply_referral(uuid, text, text) to authenticated, service_role;
grant execute on function calmkids.qualify_referral(uuid, text) to service_role;
grant execute on function calmkids.mark_referral_coupon_applied(uuid, text, text) to service_role;
